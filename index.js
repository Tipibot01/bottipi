...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            ...
            + 'FN: Aidil Tipi\n' // NAMA KONTAK OWNER
            + 'ORG:TIPI BOT;\n' // NAMA CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=62895704959080+6295-7049-59080\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '!'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MOD TIPI BOT*',
					...
					userB: `──「 DAFTAR 」──\nHalo Kak !\nMaaf Kak Kamu belum Terdaftar Sebagai Teman tipi bot Daftar Untuk Berteman Dengan tipi bot dengam cara, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar tipi|17\n\n──「 TIPI BOT 」──`,
			...
			const ownerNumber = ["62895704959080@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"62895704059080@s.whatsapp.net"]
			const adminbotnumber = ["62895704959080@s.whatsapp.net"]
			const frendsowner = ["62895704959080@s.whatsapp.net"]
			const premium = ["62895704059080@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*TIPI GROUP*\n\nLink : https://chat.whatsapp.com/H7uZc7TNJ0K9YNIxLqhbo7*', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT TIPI ✨* 」\n*│+wa.me/6281368646011*╰──────*「 *TIPI* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  TIPI BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6281368646011*╰──────*「 *TIPI* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  TIPI BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('github.com/Tipibot/Tipi-BOT@s.whatsapp.net', options, text, {quoted: mek})
 ...
 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				 data = fs.readFileSync('./src/TIPIjokes.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                client.sendMessage(hasil, image, mek, '\`\`\`TIPIJOKES\`\`\`')
				break
					case 'kata':
					if (!isPublic) return reply(mess.only.public)
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  TIPI BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner TIPI!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin TIPI!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
